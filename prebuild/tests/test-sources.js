...
const sourcesTest = (incoming) => {
	...
	Object.entries($info).forEach((pair) => {
		...
		if(file.datatype === "race" || file.link === "class" || file.link === "sidekick" || file.link === "kinetic") {
			// Race files and classes and sidekicks and kinetic talents have unpredictable sources, so don't test them.
			return;
		}
		const msg = [];
